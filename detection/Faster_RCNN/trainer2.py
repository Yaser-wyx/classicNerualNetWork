...
class FasterRCNNTrainer(nn.Module):
    ...
    def set_mode(self, mode="train"):
        assert mode in ["train", "eval"], "Mode should only be train or eval"
        if mode is "train":
    ...
    def forward(self, images, gt_bboxes, gt_labels):
        ...
        # 经过rpn生成roi
        # rpn_offset_scale_pred, rpn_bbox_bf_pred, rois, rois_idx, anchors = self.faster_rcnn.rpn(features, image_size)
        rois, rois_idx, anchors, rpn_offset_scale_pred, rpn_bbox_bf_pred = self.faster_rcnn.rpn(features, image_size)

        rpn_offset_scale_pred = rpn_offset_scale_pred[0]
        rpn_bbox_bf_pred = rpn_bbox_bf_pred[0]
        # rois = torch.tensor(rois, device=self.device)
        # rois_idx = torch.tensor(rois_idx, device=self.device)
        ...
        rpn_reg_loss = cal_reg_loss(rpn_offset_scale_pred.contiguous(), rpn_offset_scale_gt, rpn_label_gt,
                                    self.rpn_sigma)
